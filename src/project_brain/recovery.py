...
class RecoveryManager:
    def __init__(self, store: TaskStore, worktrees: WorktreeManager) -> None:
        self.store = store
        ...
        *,
        ...
        for task in tasks:
            if task["status"] != TaskStatus.RUNNING.value:
                ...
                continue
            results.append(self._reconcile_one(task, execute=execute))
        return results

    def _reconcile_one(self, task: dict[str, Any], *, execute: bool) -> dict[str, Any]:
    ...
    def _classify(
        ...
        if branch != record["branch"] or head != expected or status or conflicts:
            return (
                TaskStatus.FAILED,
                "Interrupted worktree has branch, HEAD, file, or conflict mutations; retained for forensics",
            )
