...
class HelperPackagingTests(unittest.TestCase):
    ...
    def test_pyinstaller_build_is_single_file_and_excludes_cli_extras(self) -> None:
        ...
        self.assertIn('"PROJECT_BRAIN_BUILD_SHA"', spec)
        self.assertIn('mkdtemp(prefix="project-brain-build-info-")', spec)
        self.assertIn('(str(build_info), "project_brain")', spec)
    ...
    def test_build_script_validates_executable_and_exact_version(self) -> None:
        ...
        self.assertIn('helper build requires an exact clean Git source', script)
        self.assertIn('BUILD_SHA="$(git -C "$ROOT" rev-parse HEAD)"', script)
        self.assertIn('PROJECT_BRAIN_BUILD_SHA="$BUILD_SHA"', script)
