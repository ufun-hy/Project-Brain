...
Updated: 2026-07-15
Review inputs: `Project-Brain-Core-MVP-PR12-Review-v3.md` and independent review
...
| Crash recovery | Popen process groups, child PID/PGID, background heartbeat, no duplicate attempt while child lives, and explicit group termination | `test_codex_adapter`, `test_recovery` |
...
| Verification mutation | A Git seal blocks file, commit, branch, origin, fetch, remote/local default-ref, and conflict mutations before push | `test_repository_seal` |
...
| Migrations and permissions | Atomic schema v3 migration/backfill, future-schema rejection, `0700` dirs and `0600` state/artifacts | `test_migrations`, `test_security` |
