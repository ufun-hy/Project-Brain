...
|---|---|---|
| Core/service regression | `PYTHON_BIN=... bash scripts/verify-core.sh` | Local pass, 178 tests |
...
| Swift adapter/model tests | helper rollback, JSON models, status mapping, typed argv, onboarding restore, redaction | macOS CI required |
...
| Config plan/apply | CLI `--plan` is read-only; typed adapter uses separate plan/apply calls | Local pass |
| Service idempotency/data preservation | fixed plist argv/PATH, reinstall, status, stop, restart, uninstall tests | Local pass |
