...
| `project_brain_tasks_redispatch` | write | Explicitly authorize one `needs_changes` retry at the exact current remote head, or at a retained canonical-clean local candidate after pre-publication verification failure; the backward-compatible `expected_remote_head_sha` field carries that exact candidate SHA on the local path; it does not dispatch |
