...
def main() -> int:
    ...
    review=sub.add_parser("review"); review.add_argument("task_id"); review.add_argument("decision",choices=["accepted","needs_changes"]); review.add_argument("--reason",required=True)
    ...
    if args.cmd=="list":
        ...
        if not args.reason.strip(): p.error("--reason must be non-empty")
        r=store.load(args.task_id); transition(r,args.decision,f"Review decision: {args.decision}",review_decision=args.decision,review_reason=args.reason.strip(),reviewed_at=datetime.now(timezone.utc).isoformat()); store.save(r); print(f"{args.task_id}: {args.decision}")
    return 0
