...
from gmail_callback import CallbackOutbox, FakeCallback
import status_cli
...
class StatusTests(unittest.TestCase):
 ...
 def test_callback_outbox_persistence_retry_and_idempotency(self):
  root=Path(self.tmp.name); outbox=CallbackOutbox(root); r=new_record("cb","P","T"); transition(r,"failed","failed")
  self.assertEqual(outbox.enqueue({"sender":"a","subject":"s"},r),outbox.enqueue({"sender":"a","subject":"s"},r))
  failing=FakeCallback(RuntimeError("insufficient scope: reauthorize")); items=outbox.deliver_pending(failing)
  self.assertEqual(len(items),1); self.assertEqual(items[0]["attempts"],1); self.assertTrue(items[0]["reauthorization_required"])
  data=outbox.load(); data["items"][items[0]["key"]]["next_attempt_at"]="2000-01-01T00:00:00+00:00"
  from task_status import atomic_json; atomic_json(outbox.path,data)
  ok=FakeCallback(); delivered=outbox.deliver_pending(ok); self.assertIsNotNone(delivered[0]["delivered_at"]); self.assertEqual(len(ok.sent),1)
  outbox.deliver_pending(ok); self.assertEqual(len(ok.sent),1)
 def test_record_validation(self):
  r=new_record("m","P","T"); r["state"]="bogus"
  with self.assertRaises(ValueError): self.store.save(r)
