...
@MainActor final class TaskModel:ObservableObject {
 ...
 func start(){ guard timer==nil else{return}; load(); if !requestedAuthorization {requestedAuthorization=true; UNUserNotificationCenter.current().requestAuthorization(options:[.alert,.sound]){_,_ in}}; timer=Timer.scheduledTimer(withTimeInterval:5,repeats:true){_ in Task{@MainActor in self.load()}} }
